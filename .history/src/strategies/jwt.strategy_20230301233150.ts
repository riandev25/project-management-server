...
export const getToken = (user: { _id: string }): string => {
  // This helps us to create the JSON Web Token
  return jwt.sign(user, String(process.env.JWT_SECRET), { expiresIn: '30d' });
};
...
passport.use(
  new JwtStrategy(opts, async (jwt_payload, done) => {
    ...
    const user = await UserJwt.findOne({ _id: jwt_payload._id });
    try {
      console.log(user);
